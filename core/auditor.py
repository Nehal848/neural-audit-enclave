...
def _get_conn():
    """
    Returns a SQLite connection with a 10-second busy timeout and 
    WAL journal mode to prevent 'database is locked' errors under 
    concurrent async access from the FastAPI/uvicorn server.
    """
...
class EnclaveAuditor:
    ...
    def commit_audit_log(
        ...
        with _get_conn() as conn:
            ...
            """, (modality, format, champion, confidence, action, dataset_hash, uploaded_by,
                  target_column, problem_type, doctor_notes, model_version))
    ...
    def register_user(username: str, password: str, phase: str, role: str) -> bool:
        try:
            with _get_conn() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO users (username, password, phase, role) VALUES (?, ?, ?, ?)",
                    (username, password, phase, role)
                )
                conn.commit()
            return True
    ...
    def verify_user(username: str, password: str, phase: str, role: str) -> bool:
        with _get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT COUNT(*) FROM users WHERE username = ? AND password = ? AND phase = ? AND role = ?",
                (username, password, phase, role)
            )
